...
def PassDetails(window, list_no_pass, count_pass):
    ...
    luggage_lim_lbl = CTkLabel(luggage_lim, text='Luggage Limit', font=('Georgia', 22, 'bold'), 
                        text_color='#A6ACAC'
                        )
    luggage_lim_lbl.pack()

    luggage_lim_frm = CTkFrame(luggage_lim, fg_color='transparent')
    luggage_lim_frm.pack(pady=15, padx=10)
    ...
    for i in range(3):
        ...
        lug_txt_i = CTkLabel(luggage_lim_frm, text=lug_txt[i], font=('ink free', 20, 'bold'), 
                              text_color='#A6ACAC'
                        )
        lug_txt_i.grid(row=i, column=0, sticky='w', padx=5 ,pady=15)
        
        arr_text_i = CTkLabel(luggage_lim_frm, text = ' --> ', font=('georgia', 18, 'bold'),
                              text_color='#A6ACAC'
                              )
        arr_text_i.grid(row=i, column=1, sticky='w', padx=5 ,pady=15)
                        
        lug_amt_i = CTkLabel(luggage_lim_frm, text=lug_txt_amt[i], font=('ink free', 20, 'bold'), 
                              text_color='#A6ACAC'
                        )
        lug_amt_i.grid(row=i, column=2, sticky='w', padx=5 ,pady=15)
    ...
    for j in range(3):
        ...
        for k in range(no_l[j]):
                ...
                pas_lbl_0 = CTkLabel(frm_head, text=txt_pas,font=font_head, text_color='black')
